        ...
        <div class="card-body">
          ...
          <div class="row mt-2">
            <div class="col-md-12">
              ...
              <button class="btn btn-primary btn-sm ms-2" (click)="exportToCSV()">
          ...
          <div class="col-md-3">
            <div class="card text-center">
              <div class="card-body">
                ...
                <p class="card-text display-6 text-danger">{{ getRedeemedPoints() }}</p>
              </div>
            </div>
          </div>
          <div class="col-md-3">
...
export class AllScoreHistoryComponent implements OnInit {
  ...
  filterRecords() {
    this.filteredHistory = this.allHistory.filter(record => {
      const matchesSearch = !this.searchTerm ||
        (record.visitorName?.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
         record.visitorId?.toLowerCase().includes(this.searchTerm.toLowerCase()));

      const matchesOrigin = !this.selectedOrigin || record.origin === this.selectedOrigin;

      const matchesStrategy = !this.selectedStrategy ||
        record.strategyName.toLowerCase().includes(this.selectedStrategy.toLowerCase());
  ...
  getUniqueVisitors(): number {
    const uniqueIds = new Set(this.filteredHistory.map(r => r.visitorId).filter(id => id));
  ...
  getOriginBadgeClass(origin: string): string {
    switch (origin) {
      ...
      case 'SpecialMission': return 'bg-warning';
  ...
  exportToCSV() {
    ...
    link.href = url;
    link.download = `score-history-${new Date().toISOString().split('T')[0]}.csv`;
    link.click();
